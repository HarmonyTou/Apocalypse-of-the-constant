...
local function DoRegen(inst, owner)
    if owner.components.sanity ~= nil and owner.components.sanity:IsInsanityMode() then
        ...
        inst.components.setbonus:IsEnabled(EQUIPMENTSETNAMES.KNIGHTMARE) and TUNING.ARMOR_DREADSTONE_REGEN_SETBONUS or 1 -- Cassielu: need constant for new EQUIPMENTSETNAMES?
        local rate = 1 /
