...
local function OnUnequip(inst, owner)
    ...
    if owner.components.grue ~= nil then
        owner.components.grue:RemoveImmunity("nightmarehelm")
    end
end
...
local swap_data_broken = { bank = "helm_nightmare", anim = "broken" }
...
local function CreateFxFollowFrame(i)
	...
	inst.AnimState:SetBank("nightmarehelm")
	inst.AnimState:SetBuild("helm_nightmare")
...
return MakeHat("nightmare"),
    MakeFollowFx("nightmarehelm_fx", {
        createfn = CreateFxFollowFrame,
        framebegin = 1,
        frameend = 3,
        isfullhelm = true,
        assets = { Asset("ANIM", "anim/helm_nightmare.zip") },
    })
